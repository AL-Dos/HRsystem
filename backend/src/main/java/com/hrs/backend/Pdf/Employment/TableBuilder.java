...
import com.hrs.backend.DTOs.EmploymentRow;
import com.hrs.backend.Pdf.PdfUtil.Fonts;

public class TableBuilder {
    public void build(Document doc, List<EmploymentRow> rows) throws DocumentException {
        ...
        addHeaderCell(table, "Inclusive Dates", headerFont);
        ...
        if (rows == null || rows.isEmpty()) {
            ...
            for (EmploymentRow r : rows) {
                boolean isPresent = r.getInclusiveDates() != null && r.getInclusiveDates().toLowerCase().contains("present");
                Font rowFont = isPresent ? boldFont : bodyFont; 
                addBodyCell(table, String.valueOf(counter++), bodyFont, Element.ALIGN_CENTER);
                addBodyCell(table, safe(r.getPosition()), bodyFont, Element.ALIGN_LEFT);
                addBodyCell(table, safe(r.getStatus()), bodyFont, Element.ALIGN_LEFT);
                addBodyCell(table, safe(r.getInclusiveDates()), rowFont, Element.ALIGN_LEFT);
            }
        }
        doc.add(table);
    }
